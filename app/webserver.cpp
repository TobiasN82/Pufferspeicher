...
HttpServer server;
extern String StrT1, StrT2; // Sensors string values
...
void onConfiguration(HttpRequest &request, HttpResponse &response)
{
	...
	if (request.getRequestMethod() == RequestMethod::POST)
	{
		...
		{
			cfg.AddT1 = request.getPostParameter("T1").toFloat();
			cfg.AddT2 = request.getPostParameter("T2").toFloat();
	...
	vars["T1"] = String(cfg.AddT1, 2);
	vars["T2"] = String(cfg.AddT2, 2);
...
void onApiSensors(HttpRequest &request, HttpResponse &response)
{
	...
	sensors["temperature1"] = StrT1.c_str();
	sensors["temperature2"] = StrT2.c_str();
